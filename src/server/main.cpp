...
bool parseArgs(const CmdLine& cmd,
               ...
               int& workers,
               int& queue_size,
               std::string& content_filepath) {
  port = 8888;
  request_length = 1024;
  workers = 4;
  queue_size = 10;
  content_filepath = "./resource_content.conf";
  ...
  if (!s_content_filepath.empty()) {
    content_filepath = s_content_filepath;
  }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  main_thread(port, max_request_size, workers, queue_size, content_filepath);
